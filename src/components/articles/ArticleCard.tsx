...
export default function ArticleCard({ article }: ArticleCardProps) {
  ...
  return (
    <article className="group relative bg-white rounded-3xl shadow-xl hover:shadow-3xl transition-all duration-500 overflow-hidden border border-gray-100 hover:border-emerald-200 transform hover:scale-105 hover:-translate-y-2">
      ...
      <div className="p-10">
        <h3 className="font-bold text-gray-900 mb-6 group-hover:text-emerald-600 transition-colors leading-tight text-2xl">
          {article.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-3 mb-10">
          {article.tags.slice(0, 3).map((tag) => (
            <span key={tag} className="px-4 py-2 bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 rounded-xl text-sm font-semibold hover:from-emerald-100 hover:to-teal-100 hover:text-emerald-700 transition-all duration-300 transform hover:scale-105 shadow-sm">
              #{tag}
            </span>
          ))}
        </div>
        ...
        <Link 
          ...
          className="w-full bg-gradient-to-r from-emerald-600 via-teal-600 to-green-600 hover:from-emerald-700 hover:via-teal-700 hover:to-green-700 text-white py-5 px-8 rounded-2xl font-bold text-lg transition-all duration-300 transform hover:scale-105 hover:shadow-2xl flex items-center justify-center gap-4 group"
        >
